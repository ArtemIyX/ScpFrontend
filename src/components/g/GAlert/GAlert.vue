...
import { buildGAlertClasses, type GAlertEmits, type GAlertProps } from './GAlert'

defineOptions({ inheritAttrs: false })

const props = withDefaults(defineProps<GAlertProps>(), {
  preset: 'quiet',
  variant: 'soft',
  size: 'md',
  width: 'auto',
...
const emit = defineEmits<GAlertEmits>()
...
<template>
  ...
  >
    <div class="galert__accent" aria-hidden="true"></div>

    <div class="galert__shell">
      <div v-if="slots.icon || icon || iconSrc" class="galert__icon" aria-hidden="true">
        <slot name="icon">
        ...
        <header v-if="slots.header || title || closable" class="galert__header">
          <slot name="header" :close="onClose">
            ...
            <div class="galert__chrome">
              ...
              >
                <template #icon>
                  <span class="galert__close-icon" aria-hidden="true"></span>
                </template>
              </GButton>
            </div>
          </slot>
...
<style scoped src="./GAlert.css"></style>
